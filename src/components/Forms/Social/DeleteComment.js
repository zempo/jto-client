...
const DeleteComment = ({ item, cancel }) => {
  ...
  const handleDelete = async (e) => {
    ...
    try {
      ...
      let commentsResult = await listCardComments.get(
        `/${commentToDelete.data.payload.card_id}`
      );
      // eslint-disable-next-line
      let deletedComments = await deleteCardComment(
        commentsResult.data.payload
      );
    ...
    <div
      className={
        resStatus === 0 || resStatus === 204
          ? "comment-delete"
          : "comment-delete shake"
      }
    >
      <h2>Are you sure?</h2>
      {resStatus === 0 || resStatus === 204 ? null : (
