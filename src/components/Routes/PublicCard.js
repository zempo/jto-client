...
import { listCardActions } from "../../services/endpoints-service";
import { useModal } from "../../hooks/use-modal";
import BottomModal from "../../modals/BottomModal";
// create nice back-button
import { PostReaction, ToggleReaction } from "../Forms/Social/Action";
import {
  JtoSection,
  TimeStamp,
  CardPages,
...
const PublicCard = (props) => {
  const {
    ...
    toggle: toggleCommentAdd,
  } = useModal();
  const {
    isShowing: isShowingCommentEdit,
    toggle: toggleCommentEdit,
  } = useModal();
  const {
    isShowing: isShowingCommentDelete,
    toggle: toggleCommentDelete,
  } = useModal();
  const {
    value: { user },
  ...
  useEffect(() => {
    ...
    if (props.location.state !== undefined) {
      ...
      const cardFound = async () => {
        setLoading(true);
        try {
          const getAction = await listCardActions.get(`/${item}`);
          // console.log(getAction);
          if (getAction.data.length > 0) {
            setHasActed(getAction.data[0]);
          }
  ...
  return (
    <div className='public-card-page'>
      ...
      {card.date_modified == undefined ? (
        <h2 className='animated-h2'>
          ...
          <span className='username'>
            {cardAuthor.user_name === user.user_name
              ? "you, "
              : cardAuthor.user_name}
          </span>{" "}
      ...
      <JtoSection
        className='jto-card public-card'
      ...
      <JtoSection className='jto-reactions'>
        {hasActed.action_liked === undefined ||
        hasActed.action_saved === undefined ? (
          <PostReaction item={card.id} toggleCommentAdd={toggleCommentAdd} />
        ) : (
          <ToggleReaction
            item={card.id}
            liked={hasActed.action_liked}
            shared={hasActed.action_saved}
              ...
              <li className='jto-comment' key={i}>
                ...
                <ul className='comment-items'>
                  ...
                  {comment.date_modified == undefined ? (
                    ...
                    <li className='comment-item date'>
                      <TimeStamp date={comment.date_modified}></TimeStamp>
                      &nbsp;ago (Edited)
                    </li>
                  )}
                    ...
                    <nav className='jto-dot-menu'>
                      ...
                      >
                        ...
                        <div
                          ...
                          onClick={() => openEdit(comment.id, comment.body)}
                        >
